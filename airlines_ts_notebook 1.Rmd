...
# Convert the pivot_airlines_sum to an xts object
pivot_airlines_xts <- xts(
  x = pivot_airlines_sum[,-1], # Exclude the datetime column for the data argument
  order.by = pivot_airlines_sum$datetime # Use the datetime column for the index
)

# Now, pivot_airlines_xts is an xts object with a similar structure to a data frame
...
# Plotting the xts object using autoplot
p <- autoplot(pivot_airlines_xts) +
  labs(title = "Airline Passenger Traffic Over Time", 
...
# Convert xts object to a long data frame for use with ggplot2
pivot_airlines_long <- fortify.zoo(pivot_airlines_xts)

# Check names of the resulting data frame to ensure correct column naming
print(names(pivot_airlines_long))

# Assuming the column names are 'Index' for dates and the rest are airline names
pivot_airlines_long <- pivot_airlines_long %>%
  tidyr::pivot_longer(
    -Index, 
    names_to = "Airline", 
    values_to = "Total_Passengers"
...
# Now use ggplot2 with the corrected column names
p <- ggplot(pivot_airlines_long, aes(x = Index, y = Total_Passengers, color = Airline)) +
  geom_line() +
  labs(title = "Airline Passenger Traffic Over Time",
       x = "Date", y = "Total Passengers", color = "Airline") +
  theme_minimal() +
  theme(legend.title = element_blank())
...
We will subset `pivot_airlines_xts` for southwest airlines given by `AIRLINE_ID = 19393`.

```{r}
# Assuming 'pivot_airlines_xts' is the xts object created from pivot_airlines_sum
# Extract the column for Southwest Airlines, ensuring it's univariate
southwest_series <- pivot_airlines_xts[,"19393", drop = TRUE]  # Ensure it's a vector

# Convert the xts series to a ts object
start_year <- format(index(southwest_series)[1], "%Y")
start_month <- format(index(southwest_series)[1], "%m")
southwest_ts <- ts(southwest_series, start = c(as.numeric(start_year), as.numeric(start_month)), frequency = 12)

# Recheck if it's now a univariate time series
